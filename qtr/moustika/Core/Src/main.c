...
int main(void)
{
  ...
  while (1)
  {
      ...
      for (int i = 0; i < 8; i++)
      {
          uint16_t adcVal = qtrValues[i];

          // --- SCALING ---
          // Map 0-4095 to 0-48 pixels height
          uint8_t barHeight = (adcVal * 48) / 4095;
          if(barHeight > 48) barHeight = 48; // Clip safety

          // --- POSITIONING ---
          // 8 bars, 16px wide slot each.
          // We draw a 14px wide bar to leave a 2px gap.
          uint8_t xPos = i * 16;

          // OLED Y=0 is top. To draw from bottom (Y=64) up:
          // Start Y = 64 - Height
          uint8_t yPos = 64 - barHeight;

          // --- DRAWING ---
          // Use Filled Rectangle for solid white bars
